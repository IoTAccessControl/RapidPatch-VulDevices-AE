...
static int add_normal_bkpt(uint32_t inst_addr) {
	...
	if (halting_debug_enabled()) {
		DEBUG_LOG("ERROR: Failed to add debug-mon bpkt. As the halting debug mode is enabled (Please reboot your device.).\n");
		return -1;
	}
...
//	*SCB_DFSR = (1 << 1);
	*scb_dfsr = (1 << 2);
	...
	if (halting_debug_enabled()) {
		...
		// volatile uint32_t *dhcsr = (uint32_t *)0xE000EDF0;
		// *dhcsr = (0xA05F<<16);
		DEBUG_LOG("**WARNING**: Please power off you devices and retry!\n");
		return;
	}
	...
	*demcr |= DCB_DEMCR_MON_EN; 
	...
	fpb_enable();
	DEBUG_LOG("debug_monitor_init success dhcsr: %u\n", *demcr);
...
__NAKE void debug_monitor_handler(void) {
	__asm__ __volatile__("and r0, lr, #4");
	__asm__ __volatile__("cmp r0, #0");
	__asm__ __volatile__("ite eq");
	__asm__ __volatile__("mrseq r0, msp");
...
void trigger_fpb_patch() {
	DEBUG_LOG("trigger_fpb_patch\n");
	DEBUG_LOG("Call buggy func before fpb patch!\n");
	RawBuggyFunc();
	...
	DEBUG_LOG("Call buggy func after fpb patch!\n");
	RawBuggyFunc();
}
...
void trigger_debugmon_patch(void) {
	...
	add_breakpoint((uint32_t) test_dynamic_bug);

	DEBUG_LOG("Call buggy func after kprobe patch!\n");
	test_func();
}
